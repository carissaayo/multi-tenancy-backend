...
let ChannelMembershipService = ChannelMembershipService_1 = class ChannelMembershipService {
    ...
    async addMemberToChannel(channelId, memberId, workspaceId) {
        const workspace = await this.workspaceRepo.findOne({
            where: { id: workspaceId },
        });
        if (!workspace) {
        ...
        const isChannelMember = await this.isUserMember(channelId, memberId, workspaceId);
        if (isChannelMember) {
            throw custom_errors_1.customError.badRequest('Member is already in this channel');
        }
        const sanitizedSlug = this.workspacesService.sanitizeSlugForSQL(workspace.slug);
        const schemaName = `workspace_${sanitizedSlug}`;
        try {
        ...
        ON CONFLICT (channel_id, member_id) DO NOTHING
        `, [channelId, memberId]);
            this.logger.log(`Member ${memberId} added to channel ${channelId} in workspace ${workspaceId}`);
            const channel = await this.channelQueryService.findChannelById(channelId, workspaceId);
            if (!channel) {
                throw custom_errors_1.customError.internalServerError('Failed to retrieve channel after adding member');
            }
            return channel;
        }
        catch (error) {
            this.logger.error(`Error adding member ${memberId} to channel ${channelId}: ${error.message}`);
            if (error.message?.includes('does not exist')) {
                throw custom_errors_1.customError.internalServerError('Workspace schema not found');
            }
    ...
    async leaveChannel(channelId, memberId, workspaceId) {
        const workspace = await this.workspaceRepo.findOne({
            where: { id: workspaceId },
        });
        if (!workspace) {
        ...
        const isChannelMember = await this.isUserMember(channelId, memberId, workspaceId);
        if (!isChannelMember) {
            throw custom_errors_1.customError.badRequest('You are not a member of this channel');
        }
        const sanitizedSlug = this.workspacesService.sanitizeSlugForSQL(workspace.slug);
        const schemaName = `workspace_${sanitizedSlug}`;
        try {
        ...
        `, [channelId, memberId]);
            if (!result || result.length === 0) {
                throw custom_errors_1.customError.badRequest('Failed to leave channel');
            }
            this.logger.log(`Member ${memberId} left channel ${channelId} in workspace ${workspaceId}`);
            return true;
        }
        catch (error) {
            this.logger.error(`Error removing member ${memberId} from channel ${channelId}: ${error.message}`);
            if (error.message?.includes('does not exist')) {
                throw custom_errors_1.customError.internalServerError('Workspace schema not found');
            }
            throw custom_errors_1.customError.internalServerError('Failed to leave channel');
        }
    }
    async removeMemberFromChannel(channelId, targetMemberId, workspaceId) {
        const workspace = await this.workspaceRepo.findOne({
            where: { id: workspaceId },
        });
        if (!workspace) {
        ...
        const isChannelMember = await this.isUserMember(channelId, targetMemberId, workspaceId);
        if (!isChannelMember) {
            throw custom_errors_1.customError.badRequest('Member is not in this channel');
        }
        const sanitizedSlug = this.workspacesService.sanitizeSlugForSQL(workspace.slug);
        const schemaName = `workspace_${sanitizedSlug}`;
        try {
        ...
        `, [channelId, targetMemberId]);
            if (!result || result.length === 0) {
                throw custom_errors_1.customError.badRequest('Failed to remove member from channel');
            }
            this.logger.log(`Member ${targetMemberId} removed from channel ${channelId} in workspace ${workspaceId}`);
            return true;
        }
        catch (error) {
            this.logger.error(`Error removing member ${targetMemberId} from channel ${channelId}: ${error.message}`);
            if (error.message?.includes('does not exist')) {
                throw custom_errors_1.customError.internalServerError('Workspace schema not found');
            }
            throw custom_errors_1.customError.internalServerError('Failed to remove member from channel');
        }
    }
};
