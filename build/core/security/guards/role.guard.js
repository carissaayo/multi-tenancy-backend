...
exports.RoleGuard = exports.RequireRole = exports.WorkspaceRole = void 0;
...
const RequireRole = (...roles) => (0, common_1.SetMetadata)('roles', roles);
...
let RoleGuard = RoleGuard_1 = class RoleGuard {
    ...
    canActivate(context) {
        ...
        if (!member) {
            throw custom_errors_1.customError.forbidden('Workspace membership required');
        }
        const hasRole = requiredRoles.includes(member.role);
        if (!hasRole) {
            this.logger.warn(`Role denied: User has role '${member.role}' but needs one of [${requiredRoles.join(', ')}]`);
            throw custom_errors_1.customError.forbidden(`Required role: ${requiredRoles.join(' or ')}`);
        }
